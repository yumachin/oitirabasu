...
export default function SpecSubjects() {
  const [specSubjects, setSpecSubjects] = useState<NewSubject[]>([]);

  useEffect(() => {
    const getSubjects = async () => {
      const API_URL = process.env.NEXT_PUBLIC_API_URL;
      const res = await fetch(`${API_URL}/api/LpSubject/3`, {
        cache: "no-store"
      });
      const data = await res.json();
      setSpecSubjects(data.subjects);
    };
